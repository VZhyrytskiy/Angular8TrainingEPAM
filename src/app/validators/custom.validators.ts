...
export function checkServiceLevel(c: AbstractControl, min: number = 1, max: number = 2): { [key: string]: boolean } | null {
...
export function checkPhone(c: AbstractControl): { [key: string]: boolean } | null {
  if (c.value !== undefined && Number.isNaN(c.value)) {
    return {
      phone: true
    };
  } else {
